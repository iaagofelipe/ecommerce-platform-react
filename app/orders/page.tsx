...
import { Search, Filter, Calendar, Package, Eye, XCircle } from "lucide-react"
...
export default function OrdersPage() {
  ...
  const { data: ordersData, isLoading, error } = useOrdersByCustomer(customerId, customerId ? filters : undefined)

  const handleCustomerIdSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (customerId.trim()) {
      // Reset filters when changing customer
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                {ordersData && <Badge variant="outline">{ordersData.totalElements} pedidos encontrados</Badge>}
              </div>
            </CardHeader>
            <CardContent>
              {isLoading && (
              ...
              {error && (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mb-4">Não foi possível carregar os pedidos deste cliente.</p>
                  ...
                  {ordersData.totalPages > 1 && (
                    <div className="flex items-center justify-between mt-6">
                      <p className="text-sm text-muted-foreground">
                        Página {ordersData.number + 1} de {ordersData.totalPages}
                      </p>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handlePageChange(ordersData.number - 1)}
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => handlePageChange(ordersData.number + 1)}
                          disabled={ordersData.number >= ordersData.totalPages - 1}
                        >
                          Próxima
                        </Button>
                      </div>
