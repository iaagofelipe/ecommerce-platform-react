...
import type {CreateOrderRequest, Order, PaginatedOrders} from "@/lib/schemas"
...
export function useOrdersByCustomer(
    customerId: string,
    filters?: { page?: number; size?: number; status?: string; from?: string; to?: string },
) {
    return useQuery({
        ...
        queryFn: async (): Promise<PaginatedOrders> => {
            ...
            const url = `/orders/customer/${customerId}?${params}`
            const res = await apiClient.get(url)
            return res.data
        },

        onSuccess: (data) => {
            console.log("[useOrdersByCustomer] SUCCESS:", {
        ...
        refetchInterval: false, // deixe false pra depurar; ligue depois se quiser polling
    })
}
